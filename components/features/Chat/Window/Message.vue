  ...
  <div class="ChatWindowMessage relative">
    <div :class="styles.contentWrapper" class="w-[85%]">
      <div
        ...
        class="ChatWindowMessage__header flex items-center gap-0.5 truncate text-[0.6rem]"
      >
        <template v-if="props.messageType === EMessageHost.another">
          <b class="max-w-[190px] truncate">
            {{ props.user.name }}
          </b>
      ...
      <div :class="styles.message" class="ChatWindowMessage__message w-fit rounded-lg p-1 text-xs break-all">
        {{ props.message }}
      </div>
    </div>
    <NuxtLink :to="'/user/' + props.user.slug">
      <u-avatar :class="styles.avatar" class="absolute bottom-0 size-6" :src="props.user.avatar"></u-avatar>
    </NuxtLink>
  </div>
</template>
