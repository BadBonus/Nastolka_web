...
const props = withDefaults(defineProps<TButton & /* @vue-ignore */ ButtonHTMLAttributes>(), {
  variant: 'filled',
  color: 'primary',
  size: 'md',
  block: false,
...
const buttonClasses = computed(() => {
  ...
  const resultSizeClasses = props.onlyIcon ? sizeClassesOnlyIcon[props.size] : sizeClasses[props.size];

  return [...baseClasses, resultSizeClasses, variantColorClasses[props.variant][props.color], roundedClass];
});
...
<template>
  <button :type="type" :disabled="disabled || loading" :inert="disabled || loading" :class="buttonClasses" v-bind="$attrs">
    <div
      v-if="loading"
    ...
    <span :class="{ invisible: loading }" class="inline-flex items-center">
      ...
      <slot />
      <Icon :class="[iconSpacingClasses]" class="shrink-0" v-if="icon && props.iconPos === 'right'" :name="icon" />
    </span>
  </button>
</template>
